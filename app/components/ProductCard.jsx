...
export default function ProductCard({ id, title, price, image }) {
  return (
    <Link href={`/product/${id}`} className="rounded-xl overflow-hidden flex flex-col group hover:shadow-xl transition-all duration-300 bg-white border border-transparent hover:border-gray-200">
      <div className="relative aspect-[4/3] bg-gray-50 rounded-t-xl overflow-hidden">
        <Image src={image} alt={title} fill className="object-contain p-4 group-hover:scale-105 transition-transform duration-500" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" />
      </div>
